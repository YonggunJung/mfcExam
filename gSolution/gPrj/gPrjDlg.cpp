...
void CgPrjDlg::OnBnClickedBtnThread()
{
	...
	}	// 약간 수학적 내용 이해하고 넘어가면 좋긴함. 별거 아니긴함. 외워도됨

	// 이렇게 하나하나 해주는건 빡쎄니깐 위처럼
	/*rt[0].SetRect(0, 0, nImageSize, nImageSize);
	rt[1].SetRect(nImageSize, 0, nImageSize*2, nImageSize);
	rt[2].SetRect(0, nImageSize, nImageSize, nImageSize*2);
	rt[3].SetRect(nImageSize, 0, nImageSize*2, nImageSize);*/

	thread _thread0(threadProcess, this, rt[0], &nRet[0]);	// 좌상단
	thread _thread1(threadProcess, this, rt[1], &nRet[1]);	// 우상단
	thread _thread2(threadProcess, this, rt[2], &nRet[2]);	// 좌하단
	thread _thread3(threadProcess, this, rt[3], &nRet[3]);	// 우하단
